...
class TransactionController extends Controller
{
    ...
    public function transactCreateSearchData(Request $request) {
        ...
        if ($search === null) {
            ...
            $customerCheck = DB::select("SELECT cu.*, c.name city, ci.name country FROM customer_infos cu LEFT JOIN account_openings ao ON ao.customer_id = cu.id LEFT JOIN cities c ON c.id = cu.city_id LEFT JOIN country_infos ci ON ci.id = cu.country_id WHERE ((cu.contact_number = '$search' OR cu.id_number='$search') AND cu.status=1 ) OR ( ao.account_no = '$search' AND ao.status='1')");
            if ($customerCheck) {
